...
public final class GlyphPage {
  ...
  public void setupTexture() {
    try {
      ...
      data.flip();
      this.loadedTexture = (class_1044)new class_1043(class_1011.method_4324(data));
    } catch (Exception e) {
      e.printStackTrace();
    } 
  }
  ...
  public float drawChar(class_4587 stack, char ch, float x, float y, float red, float blue, float green, float alpha) {
    ...
    if (glyph == null)
      return 0.0F; 
    float pageX = glyph.x / this.imgSize;
    float pageY = glyph.y / this.imgSize;
    float pageWidth = glyph.width / this.imgSize;
    float pageHeight = glyph.height / this.imgSize;
  ...
  public float getWidth(char ch) {
    return ((Glyph)this.glyphCharacterMap.get(Character.valueOf(ch))).width;
  }
