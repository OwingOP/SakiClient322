...
public final class KeybindBox extends RenderableSetting {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int off = parentX() + 6;
        if (!this.keybind.isListening()) {
            TextRenderer.drawString(this.setting.getName() + ": " + this.setting.getName(),
                    context, off, parentY() + parentOffset() + this.offset + 9,
                    new Color(245, 245, 245, 255).getRGB());
        } else {
            TextRenderer.drawString("Listening...", context, off,
                    parentY() + parentOffset() + this.offset + 6,
                    new Color(245, 245, 245, 255).getRGB());
        }
        if (!this.parent.parent.dragging) {
            ...
            context.fill(parentX(), parentY() + parentOffset() + this.offset,
                    parentX() + parentWidth(), parentY() + parentOffset() + this.offset + parentHeight(),
