...
public final class AimAssist extends Module {
    ...
    public AimAssist() {
        ...
        addSettings(stickyAim, onlyWeapon, onLeftClick, aimAt, stopAtTargetVertical, stopAtTargetHorizontal,
                radius, seeOnly, lookAtNearest, fov, pitchSpeed, yawSpeed, speedChange, randomization,
                yawAssist, pitchAssist, waitFor, lerp, posMode);
    }
    ...
    @Override
    public void onRender3D(net.minecraft.client.util.math.MatrixStack stack, float partialTicks) {}

    @Override
    public void onRenderHud(net.mehvahdjukaar.modelfix.event.events.HudListener.HudEvent event) {
        ...
        if (stickyAim.getValue() && mc.player.getAttacking() instanceof PlayerEntity player &&
                player.distanceTo(mc.player) < radius.getValue()) {
            target = player;
        }
        ...
        Vec3d targetPos = posMode.isMode(PosMode.Normal)
                ? target.getPos()
                : target.getLerpedPos(RenderTickCounter.ONE.getTickDelta(true));

        if (aimAt.isMode(AimMode.Chest)) targetPos = targetPos.add(0, -0.5, 0);
        else if (aimAt.isMode(AimMode.Legs)) targetPos = targetPos.add(0, -1.2, 0);
        ...
        if (angleToRotation > (double) fov.getValueInt() / 2) return;

        float yawStrength = yaw / 50;
        float pitchStrength = pitch / 50;

        float yawVal = mc.player.getYaw();
        float pitchVal = mc.player.getPitch();
        if (lerp.isMode(LerpMode.Smoothstep)) {
            yawVal = (float) smoothStepLerp(yawStrength, mc.player.getYaw(), (float) rotation.yaw());
            pitchVal = (float) smoothStepLerp(pitchStrength, mc.player.getPitch(), (float) rotation.pitch());
        } else if (lerp.isMode(LerpMode.Normal)) {
            yawVal = lerp(yawStrength, mc.player.getYaw(), (float) rotation.yaw());
            pitchVal = lerp(pitchStrength, mc.player.getPitch(), (float) rotation.pitch());
        } else if (lerp.isMode(LerpMode.EaseOut)) {
            yawVal = (float) easeOutBackDegrees(mc.player.getYaw(), rotation.yaw(),
                    yawStrength * RenderTickCounter.ONE.getLastFrameDuration());
            pitchVal = (float) easeOutBackDegrees(mc.player.getPitch(), rotation.pitch(),
                    pitchStrength * RenderTickCounter.ONE.getLastFrameDuration());
        }
            ...
            if (move) {
                if (yawAssist.getValue()) {
                    if (stopAtTargetHorizontal.getValue()
                            && WorldUtils.getHitResult(radius.getValue()) instanceof EntityHitResult hitResult
                            && hitResult.getEntity() == target) {
                        return;
                    }
                    mc.player.setYaw(yawVal);
                }
    ...
    public static double easeOutBackDegrees(double start, double end, float speed) {
        ...
        return start + MathHelper.wrapDegrees(end - start) *
